...
class JobCompletionProcessor {
  ...
  async processJob(job) {
    ...
    const {context, jobId, chainId, tokenType} = job;
    console.log(`JobCompletionProcessor - ${tokenType} job [${jobId}] on chain [${chainId}]`);
    ...
    if (transactionIsConfirmed) {
      ...
      console.log(`Moving ${tokenType} job [${jobId}] on chain [${chainId}] to new status of [${jobStatus}]`);
      return this.jobQueue.addStatusAndContextToJob(chainId, jobId, jobStatus, newContext, tokenType);
    }
